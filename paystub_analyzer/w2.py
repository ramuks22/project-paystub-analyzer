...
def compare_snapshot_to_w2(
    ...
    def add_comparison(field: str, pair: AmountPair, w2_key: str, mode: str = "strict") -> None:
        comparisons.append(
            compare_amounts(
                field,
                pair.ytd,
                as_decimal(w2_data.get(w2_key)),
                tolerance,
                mode=mode,
            )
        )

    add_comparison("federal_income_tax_withheld", snapshot.federal_income_tax, "box_2_federal_income_tax_withheld")
    add_comparison("social_security_tax_withheld", snapshot.social_security_tax, "box_4_social_security_tax_withheld")
    add_comparison("medicare_tax_withheld", snapshot.medicare_tax, "box_6_medicare_tax_withheld")
    add_comparison(
    ...
    all_states = sorted(set(snapshot.state_income_tax.keys()) | set(state_boxes.keys()))
    for state in all_states:
        ...
        comparisons.append(
            compare_amounts(
                ...
                paystub_state.ytd if paystub_state else None,
                w2_state_tax,
                tolerance,
            )
        )
