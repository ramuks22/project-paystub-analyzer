    ...
    for snapshot in snapshots:
        ...
        for state in states:
            ...
            if prev_pair.ytd is None or curr_pair.ytd is None:
                continue
            if curr_pair.ytd + tolerance < prev_pair.ytd:
                note = f"{state} state YTD decreased {format_money(prev_pair.ytd)} -> {format_money(curr_pair.ytd)}"
                record_note(notes_by_file, snapshot.file, note)
                issues.append(
                    ConsistencyIssue(
                        severity="warning",
            ...
            expected_ytd = (prev_pair.ytd + curr_pair.this_period).quantize(Decimal("0.01"))
            if abs(curr_pair.ytd - expected_ytd) > tolerance:
                note = f"{state} parsed YTD {format_money(curr_pair.ytd)} vs calculated {format_money(expected_ytd)}"
                record_note(notes_by_file, snapshot.file, note)
                issues.append(
                    ConsistencyIssue(
                        severity="warning",
